...
<body>
    ...
    $(document).ready(function () {
        // Fetch and display bakeshop applications
        $.ajax({
            ...
            success: function (data) {
                ...
                data.forEach(function (bakeshop) {
                    rows += `<tr id="row-${bakeshop.id}">
                                ...
                                <td><a href="#" class="view-image" data-image="${bakeshop.business_permit_photo}">View</a></td>
                                <td><a href="#" class="view-image" data-image="${bakeshop.health_permit_photo}">View</a></td>
                                <td><a href="#" class="view-image" data-image="${bakeshop.bakeshop_photo}">View</a></td>
                                <td id="status-${bakeshop.id}">${bakeshop.status}</td>
                                <td>
                                    <button class="btn btn-success btn-sm" onclick="approveApplication(${bakeshop.id})">Approve</button>
                                    <button class="btn btn-danger btn-sm" onclick="disapproveApplication(${bakeshop.id})">Disapprove</button>
                                </td>
                            </tr>`;
                });
                $('#bakeshop-table-body').html(rows);
            },
            error: function () {
                $('#bakeshop-table-body').html('<tr><td colspan="11">Failed to load data.</td></tr>');
            }
        });
    ...
    function approveApplication(id) {
        if (confirm('Are you sure you want to approve this application?')) {
            $.ajax({
                ...
                error: function () {
                    alert('Error occurred while approving the application.');
                }
            });
        }
    }
    ...
    function disapproveApplication(id) {
        if (confirm('Are you sure you want to disapprove this application?')) {
            $.ajax({
                ...
                error: function () {
                    alert('Error occurred while disapproving the application.');
                }
            });
        }
    }
