...
// Fetch bread items along with bakeshop names from the database
$query = "
    SELECT bread_items.id, bread_items.name, bread_items.description, bread_items.price, bread_items.stock, bread_items.image, bakeshop_applications.business_name 
    FROM bread_items 
    JOIN bakeshop_applications ON bread_items.bakeshop_id = bakeshop_applications.id
";
                ...
                <tbody>
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<td><img src='../" . $row['image'] . "' alt='Bread Image' class='img-fluid' style='max-width: 100px; cursor: pointer;' onclick='showImageModal(\"../" . $row['image'] . "\")'></td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='7'>No bread items found</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
